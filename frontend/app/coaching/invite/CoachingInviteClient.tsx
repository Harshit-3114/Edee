 ...
 *
 * `initialInvites` is whatever the server already fetched with the session cookie:
 ...
 */
export default function CoachingInviteClient({
  initialInvites,
}: {
  initialInvites: CoachingInvite[] | null;
}) {
  const [invites, setInvites] = useState<CoachingInvite[]>(initialInvites ?? []);
  ...
  const served = useRef(initialInvites !== null);
  useEffect(() => {
...
export type InitialData = CoachingInvite[];
