 ...
 *
 * `initialStudents` is the unfiltered cohort the server already fetched with
 * the session cookie - what shows before anyone searches or picks a stage.
 */
export default function CoachingStudentsClient({
  initialStudents,
}: {
  initialStudents: CohortStudent[] | null;
}) {
  const [students, setStudents] = useState<CohortStudent[]>(initialStudents ?? []);
  ...
  // Skips exactly one run: the unfiltered fetch the server already did.
  // Cleared immediately, so searching and filtering still work.
  const served = useRef(initialStudents !== null);
