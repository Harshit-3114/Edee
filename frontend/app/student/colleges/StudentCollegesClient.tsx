...
import type { College, CollegeQuery, ShortlistEntry } from '@/lib/types';

/**
 * Both lists arrive from the server: the unfiltered first page of colleges and
 * the student's shortlist, so the browser opens populated and every card
 * already knows whether it is shortlisted.
 */
export default function StudentCollegesClient({
  initialColleges,
  initialShortlist,
}: {
  ...
  const { colleges, loading, loadingMore, error, hasMore, loadMore } = useColleges(
    query,
    initialColleges,
  );
  const shortlist = useShortlist(initialShortlist);
