...
**Phase 1 scope:** Student signup, college search & shortlisting, Razorpay payment, per-college landing pages, and 4 role-based portals (Student, Platform Admin, College Admin, Coaching Center Admin)  
**Stack:** FastAPI · Next.js · PostgreSQL · Firebase Auth · Razorpay  
**Target:** 35,000 students (UG + PG), Mumbai-region AWS deployment

> **Multi-portal note:** Every college on the platform gets its own public landing page, and there are four separate authenticated portals — **Student**, **Platform Admin**, **College Admin**, and **Coaching Center Admin**. The Coaching Center Admin portal is read-focused: it lets a coaching center's staff view the admission activity (shortlists, applications, payment/application status) of only the students linked to that coaching center — never other centers' students, and never data outside admissions activity. See [§16](#16-multi-portal-architecture--college-landing-pages) for the full breakdown.
...
16. [Multi-Portal Architecture & College Landing Pages](#16-multi-portal-architecture--college-landing-pages)
...
│   │   ├── colleges/
│   │   │   └── [collegeSlug]/
│   │   │       └── page.tsx          # public landing page — one per college
│   │   ├── (student)/
│   │   │   ├── signup/
│   │   │   │   └── page.tsx
│   │   │   ├── profile/
│   │   │   │   └── page.tsx
│   │   │   ├── colleges/
│   │   │   │   └── page.tsx           # search across all college landing pages
│   │   │   ├── shortlist/
│   │   │   │   └── page.tsx
│   │   │   ├── checkout/
│   │   │   │   └── page.tsx
...
│   │   │   ├── colleges/
│   │   │   │   ├── page.tsx          # manage college records + landing pages
│   │   │   │   └── [collegeId]/
│   │   │   │       └── page.tsx
│   │   │   ├── coaching-centers/
│   │   │   │   └── page.tsx          # onboard/manage coaching centers
│   │   │   └── students/
│   │   │       └── page.tsx
│   │   ├── (college-admin)/
│   │   │   ├── login/
│   │   │   │   └── page.tsx
│   │   │   ├── dashboard/
│   │   │   │   └── page.tsx          # this college's admissions overview
│   │   │   ├── courses/
│   │   │   │   └── page.tsx          # manage own courses/seats/fees
│   │   │   ├── applications/
│   │   │   │   └── page.tsx          # applications to this college only
│   │   │   └── landing-page/
│   │   │       └── page.tsx          # edit own college landing page content
│   │   ├── (coaching-admin)/
│   │   │   ├── login/
│   │   │   │   └── page.tsx
│   │   │   ├── dashboard/
│   │   │   │   └── page.tsx          # read-only admissions activity overview
│   │   │   ├── students/
│   │   │   │   └── page.tsx          # linked students list
│   │   │   └── students/[studentId]/
│   │   │       └── page.tsx          # a linked student's shortlists/applications (read-only)
│   │   ├── layout.tsx
│   │   └── page.tsx
│   ├── components/
│   │   ├── auth/
│   │   │   ├── PhoneOTPForm.tsx
│   │   │   ├── GoogleSignIn.tsx
│   │   │   └── PortalLoginForm.tsx    # shared login form for the 3 staff portals
│   │   ├── colleges/
│   │   │   ├── CollegeCard.tsx
│   │   │   ├── CollegeFilters.tsx
│   │   │   ├── CollegeSearch.tsx
│   │   │   └── CollegeLandingPage.tsx # renders a single college's landing page
│   │   ├── shortlist/
│   │   │   └── ShortlistButton.tsx
│   │   ├── payment/
│   │   │   └── CheckoutSummary.tsx
│   │   └── admissions/
│   │       └── AdmissionActivityTable.tsx  # shared by admin/college-admin/coaching-admin views
│   ├── lib/
│   │   ├── firebase.ts
│   │   ├── api.ts
│   │   └── types.ts
...
│   │   ├── useAuth.ts
│   │   ├── useShortlist.ts
│   │   └── useRole.ts                 # reads role claim, gates portal routes
│   ├── .env.local
│   ├── .env.local.example
│   ├── next.config.ts
│   ├── tailwind.config.ts
...
def upgrade():
    ...
    # Colleges — each college gets its own public landing page at /colleges/{slug}
    ...
    # College Admins — one row per staff account, scoped to a single college
    ...
    # Coaching Center Admins — one row per staff account, scoped to a single center
    ...
    # Student ↔ Coaching Center links — grants READ-ONLY visibility into a
    # student's admissions activity to the linked coaching center only.
...
def require_role(role: str):
    ...
    Valid roles (one per Firebase custom claim, set at account creation):
      - "student"          → Student portal
      - "platform_admin"   → Platform Admin portal (full access, all colleges)
      - "college_admin"    → College Admin portal (scoped to college_admins.college_id)
      - "coaching_admin"   → Coaching Center Admin portal (read-only, scoped to
                              coaching_center_admins.coaching_center_id)
...
def require_any_role(*roles: str):
    """
    Same as require_role but allows multiple roles — e.g. an admissions-activity
    endpoint readable by platform_admin, college_admin, and coaching_admin alike,
    each scoped to their own visibility (see app/services/admissions_scope.py).
    """
    async def _check(user: dict = Depends(get_current_user)):
...
colleges
  id              UUID  PK
  name            TEXT
  slug            TEXT  UNIQUE   ← powers /colleges/[collegeSlug] landing page
  location        TEXT
  city            TEXT
  state           TEXT
  ...
  landing_gallery_urls      JSONB   ← array of image URLs
  active          BOOLEAN
  created_at      TIMESTAMPTZ
...
college_admins
  id              UUID  PK
  firebase_uid    TEXT  UNIQUE
  college_id      UUID  FK → colleges.id   ← scopes this admin to one college
  name            TEXT
  email           TEXT  UNIQUE
...
coaching_center_admins
  id                    UUID  PK
  firebase_uid          TEXT  UNIQUE
  coaching_center_id    UUID  FK → coaching_centers.id  ← scopes this admin to one center
  name                  TEXT
  email                 TEXT  UNIQUE
  active                BOOLEAN  DEFAULT true
  created_at            TIMESTAMPTZ

student_coaching_links
  id                    UUID  PK
  student_id            UUID  FK → students.id
  coaching_center_id    UUID  FK → coaching_centers.id
  linked_at             TIMESTAMPTZ
  UNIQUE(student_id, coaching_center_id)
  -- Grants a coaching center READ-ONLY visibility into that student's
  -- shortlists/applications/payment status only. Never grants write access,
  -- and never exposes students outside this link table.
...
## 16. Multi-Portal Architecture & College Landing Pages

Phase 1 ships **one public surface** (college landing pages) and **four separate authenticated portals**. They share the same backend and database but are distinct Next.js route groups with independent login flows and independent authorization scopes. Nobody logs into "the platform" generically — they land in exactly one portal based on their role claim.

### 16.1 College landing pages

- Every row in `colleges` has its own public page at `/colleges/{slug}` (see `app/(public)`-style route `frontend/app/colleges/[collegeSlug]/page.tsx`).
- No login required to view a landing page — it's the top-of-funnel page students land on from search or a shared link.
- Content per page: hero image, description, gallery, list of active courses with seats/fees (`college_courses`), and a "Shortlist" / "Apply" call-to-action that routes into the Student portal.
- Landing page content (`landing_hero_image_url`, `landing_description`, `landing_gallery_urls`) is editable only by that college's own College Admin (§16.4) or by Platform Admin — never by another college's admin.
- `colleges.slug` must be unique and URL-safe; generate it from `name` + city on creation and let Platform Admin override it.

### 16.2 The four portals

| Portal | Route group | Who | Scope |
|---|---|---|---|
| **Student** | `frontend/app/(student)/` | Applicants | Own profile, shortlists, applications, payments only |
| **Platform Admin** | `frontend/app/(platform-admin)/` | Anthropic-side / platform ops staff | Everything — all colleges, all coaching centers, all students, all admissions activity |
| **College Admin** | `frontend/app/(college-admin)/` | Staff of one college | Their own college's courses, seats, fees, landing page content, and applications *to that college only* |
| **Coaching Center Admin** | `frontend/app/(coaching-admin)/` | Staff of one coaching center | **Read-only** view of admission activity (shortlists, applications, payment/application status) for students linked to that center via `student_coaching_links` — nothing else |

Each portal has its own `/login` route. Firebase custom claims carry a single `role` (`student` / `platform_admin` / `college_admin` / `coaching_admin`) set when the account is provisioned — a user is never a member of more than one portal.

### 16.3 Authorization scoping (backend)

Each staff role resolves to a scope before any query runs:

- `platform_admin` → no filter, sees all rows.
- `college_admin` → every query is filtered by `college_id = college_admins.college_id` for the authenticated user (looked up from `college_admins.firebase_uid`).
- `coaching_admin` → every query joins through `student_coaching_links` filtered by `coaching_center_id = coaching_center_admins.coaching_center_id`, and is restricted to read-only endpoints (no `POST`/`PATCH`/`DELETE` on student, application, or payment data).

**`app/services/admissions_scope.py`** (new, referenced by `platform_admin.py`, `college_admin.py`, `coaching_admin.py`)
...
async def get_admissions_scope(
    user: dict = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> dict:
    """
    Resolves the caller's role into a query scope for admissions-activity
    endpoints. Returns a dict describing how to filter, never raw SQL —
    callers build parameterised queries from this.
    """
    role = user.get("role")
    ...
    if role == "college_admin":
        row = (await db.execute(
            text("SELECT college_id FROM college_admins WHERE firebase_uid = :uid AND active"),
            {"uid": user["uid"]},
        )).fetchone()
        if not row:
            raise HTTPException(status_code=403, detail="No active college admin record")
        return {"scope": "college", "college_id": row.college_id}

    if role == "coaching_admin":
        row = (await db.execute(
            text("SELECT coaching_center_id FROM coaching_center_admins WHERE firebase_uid = :uid AND active"),
            {"uid": user["uid"]},
        )).fetchone()
        if not row:
            raise HTTPException(status_code=403, detail="No active coaching center admin record")
        return {"scope": "coaching_center", "coaching_center_id": row.coaching_center_id, "read_only": True}

    raise HTTPException(status_code=403, detail="Role not permitted to view admissions activity")
```

### 16.4 New routers

- **`app/routers/platform_admin.py`** — CRUD on `colleges` (incl. landing page fields), `college_courses`, `coaching_centers`; unscoped read on `applications`/`shortlists`/`payments`.
- **`app/routers/college_admin.py`** — read/update own `colleges` row + `landing_hero_image_url`/`landing_description`/`landing_gallery_urls`, CRUD own `college_courses`, read own-college `applications`.
- **`app/routers/coaching_admin.py`** — read-only: list linked students (`student_coaching_links`), and each linked student's `shortlists` + `applications` + `payments.status`. No endpoint in this router accepts `POST`, `PATCH`, or `DELETE` on student data.

```python
# app/routers/coaching_admin.py — illustrative shape, not full implementation
from fastapi import APIRouter, Depends
from sqlalchemy.ext.asyncio import AsyncSession
from sqlalchemy import text
from app.db.connection import get_db
from app.services.admissions_scope import get_admissions_scope
...
async def list_linked_students(
    scope: dict = Depends(get_admissions_scope),
    db: AsyncSession = Depends(get_db),
):
    if scope["scope"] != "coaching_center":
        # platform_admin/college_admin use their own routers instead
        raise HTTPException(status_code=403, detail="Wrong portal for this endpoint")

    result = await db.execute(text("""
        SELECT s.id, s.name, s.stream
        FROM students s
        JOIN student_coaching_links scl ON scl.student_id = s.id
        WHERE scl.coaching_center_id = :center_id
    """), {"center_id": scope["coaching_center_id"]})
    return [dict(row._mapping) for row in result.fetchall()]


@router.get("/students/{student_id}/admissions-activity")
async def get_student_admissions_activity(
    student_id: str,
    scope: dict = Depends(get_admissions_scope),
    db: AsyncSession = Depends(get_db),
):
    """Shortlists + applications + payment status for ONE linked student — read only."""
    if scope["scope"] != "coaching_center":
        raise HTTPException(status_code=403, detail="Wrong portal for this endpoint")

    # Verify the link exists before returning anything
    linked = (await db.execute(text("""
        SELECT 1 FROM student_coaching_links
        WHERE student_id = :sid AND coaching_center_id = :cid
    """), {"sid": student_id, "cid": scope["coaching_center_id"]})).fetchone()
    if not linked:
        raise HTTPException(status_code=404, detail="Student not linked to this coaching center")

    applications = await db.execute(text("""
        SELECT a.id, c.name AS college_name, cc.course_name, a.status, a.created_at,
    ...
    """), {"sid": student_id})
    return {"applications": [dict(r._mapping) for r in applications.fetchall()]}
```

### 16.5 What each portal must never be able to do

- Student portal → cannot see other students' data, cannot see any portal's admin views.
- College Admin → cannot read or write another college's courses, landing page, or applications.
- Coaching Center Admin → **read-only everywhere**; cannot create/edit applications, cannot see students not linked via `student_coaching_links`, cannot see a linked student's payment credentials — only aggregate/status fields (`payments.status`, not `razorpay_signature` or raw amounts breakdown beyond what's already shown to the student).
- Every cross-portal read of admissions activity (Platform Admin, College Admin, Coaching Center Admin all touch the same `applications`/`shortlists` tables) must go through `get_admissions_scope` — no router should query these tables with a hand-rolled scope check.
